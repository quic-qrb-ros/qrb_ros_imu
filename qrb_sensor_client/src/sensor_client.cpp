...
int get_sensors_fd(int socket_fd, int fd_number)
{
  ...
  char buf[CMSG_SPACE(fd_number * sizeof(int))], iov_buf[1];
...
int get_sensors_ptr(int client_fd)
{
  ...
  for (int i = 0; i < (int)sensors_fd.size(); i++) {
    char* temp = (char*)(void*)mmap(NULL, DIRECT_CHANNEL_SHARED_MEMORY_SIZE, PROT_READ, MAP_SHARED,
                                    sensors_fd[i], 0);
    if (temp == nullptr) {
 ...
 *                  *---------*                    *---------*
 */
bool SensorClient::GetImuData(sensors_event_t** accel_ptr, sensors_event_t** gyro_ptr,
